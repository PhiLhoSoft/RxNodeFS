...
function run(optionName, observable)
{
	...
	observable.subscribe(
		function __onNext(file)
		{
			...
			if (f.stat)
			{
				f.stat = 'Size: ' + f.stat.size + ', modified: ' + f.stat.mtime.toISOString() + ', is file: ' + f.stat.isFile();
			}
			add('Next', f);
		},
